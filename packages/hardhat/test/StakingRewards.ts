...
describe("StakingRewards Contract", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should have the correct reward rate", async function () {
      ...
      expect(await stakingRewards.rewardRate()).to.equal(rewardAmount / BigInt(rewardDuration));
    });
  });
  ...
  describe("Staking", function () {
    ...
    it("Should update balances correctly when multiple users stake", async function () {
      ...
      expect(await stakingRewards.balanceOf(aliceAddress)).to.equal(aliceStakeAmount);
      expect(await stakingRewards.balanceOf(bobAddress)).to.equal(bobStakeAmount);
      expect(await stakingRewards.totalSupply()).to.equal(aliceStakeAmount + bobStakeAmount);
    });
  });
  ...
  describe("Withdrawing", function () {
    ...
    it("Should revert if withdrawing zero amount", async function () {
      await expect(stakingRewards.connect(alice).withdraw(0)).to.be.revertedWith("amount = 0");
    });
    ...
    it("Should revert if withdrawing more than staked", async function () {
      ...
      await expect(stakingRewards.connect(alice).withdraw(excessAmount)).to.be.reverted;
    });
  });
  ...
  describe("Rewards", function () {
    ...
    it("Should distribute rewards proportionally to stake amount", async function () {
      ...
      // Allow for small deviation due to block timestamp variations
      expect(aliceEarned).to.be.closeTo(expectedAliceReward, ethers.parseUnits("0.1", "ether"));
      expect(bobEarned).to.be.closeTo(expectedBobReward, ethers.parseUnits("0.1", "ether"));
    });
  ...
  describe("Reward Duration", function () {
    it("Should not allow changing duration while rewards period is active", async function () {
      ...
      await expect(stakingRewards.setRewardsDuration(rewardDuration * 2)).to.be.revertedWith(
  ...
  describe("Owner Functions", function () {
    it("Should restrict owner functions to the owner", async function () {
      await expect(stakingRewards.connect(alice).setRewardsDuration(rewardDuration)).to.be.revertedWith("not owner");

      await expect(stakingRewards.connect(alice).notifyRewardAmount(rewardAmount)).to.be.revertedWith("not owner");
    });
